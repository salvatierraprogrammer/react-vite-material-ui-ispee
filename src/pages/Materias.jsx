  ...
  return (
    <Box>
      ...
      <Box sx={{ display: 'flex', gap: 1, mb: 1.5, flexWrap: 'wrap' }}>
        <TextField select label="Año" value={filters.year} onChange={(e) => setFilters((p) => ({ ...p, year: e.target.value }))} size="small" sx={{ minWidth: 100 }}><MenuItem value="">Todos</MenuItem>{years.map((y) => <MenuItem key={y.id} value={y.id}>{y.name}</MenuItem>)}</TextField>
        <TextField select label="Materia" value={filters.subject} onChange={(e) => setFilters((p) => ({ ...p, subject: e.target.value }))} size="small" sx={{ minWidth: 130 }}><MenuItem value="">Todas</MenuItem>{subjects.map((s) => <MenuItem key={s.id} value={s.name}>{s.name}</MenuItem>)}</TextField>
        <TextField select label="Tipo" value={filters.type} onChange={(e) => setFilters((p) => ({ ...p, type: e.target.value }))} size="small" sx={{ minWidth: 100 }}><MenuItem value="">Todos</MenuItem>{materialTypes.map((t) => <MenuItem key={t.id} value={t.value}>{t.name}</MenuItem>)}</TextField>
        <TextField select label="Orden" value={filters.sort} onChange={(e) => setFilters((p) => ({ ...p, sort: e.target.value }))} size="small" sx={{ minWidth: 120 }}>{SORT_OPTIONS.map((o) => <MenuItem key={o.value} value={o.value}>{o.label}</MenuItem>)}</TextField>
      </Box>
